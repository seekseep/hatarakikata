...
export function buildPrompt(
  ...
  return [
    ...
    "{",
    "  actions: Array<",
    "    | { type: 'create', payload: { name?: string, startName?: string, endName?: string, type: 'living' | 'working' | 'feeling', startDate: string, endDate: string, tagNames: string[], strength: number, description: string | null } }",
    "    | { type: 'update', payload: { id: string, ...変更フィールドのみ（name, startName, endName, type, startDate, endDate, tagNames, strength, description）} }",
    "  >,",
    ...
    "  - name は『〜していた/〜に在籍/〜で勤務/〜に居住』のように期間が自然に読める名前にする。",
    "  - name に年号（令和・平成・昭和など）は含めない。",
    "  - 学歴は working として「◯◯高校在籍」「◯◯大学在籍」を基本形にする。",
    "  - 職歴は working として「会社/職種で勤務」を基本形にする。",
    "  - endName は省略可（退社・卒業など終了時の短いラベルを付けたい場合のみ設定）",
